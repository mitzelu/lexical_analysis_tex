...
def reducefn(k, vs):
	...
	import psycopg2
	import operator
	industries = ['beauty' , 'fashion' ,'game', 'sport' , 'food']
	from nltk.stem.lancaster import LancasterStemmer
	...
	st = LancasterStemmer()
	popular_words = sorted(r.items(), reverse = True, key = operator.itemgetter(1))
	top_3 = popular_words[:6]

	for industry in industries:

		industry = str(st.stem(industry))
		...
		for i,v in top_3:
			...
			if industry in i:
				...
				try:
				    conn=psycopg2.connect("dbname='youtube-db' user='postgres' ")
				except:
				...
				conn.rollback()
				cur.execute(("UPDATE youtube_channels SET category=(%s) WHERE title=(%s) "), (industry,k,));
				conn.commit()
				conn.close()
